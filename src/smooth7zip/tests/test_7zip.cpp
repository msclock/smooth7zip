...
TEST(verify_7zip, exported) {
    // SetLargePageMode
    EXPECT_EQ(SetLargePageMode(), S_OK);
...
public:
    ...
    TEST_Z7_COM7F_IMF(SetTotal(UInt64 size)) {
        if (total) {
            total(size);
        }
        return S_OK;
    ...
    TEST_Z7_COM7F_IMF(SetCompleted(const UInt64 *completeValue)) { return progress ? progress(*completeValue) : S_OK; }

    TEST_Z7_COM7F_IMF(GetUpdateItemInfo(UInt32 index, Int32 *newData, Int32 *newProps, UInt32 *indexInArchive)) {
        (void)index;
        if (newData) {
    ...
    TEST_Z7_COM7F_IMF(GetProperty(UInt32 index, PROPID propID, PROPVARIANT *value)) {
        NWindows::NCOM::CPropVariant prop;
    ...
    TEST_Z7_COM7F_IMF(GetStream(UInt32 index, ISequentialInStream **inStream)) {
        COM_TRY_BEGIN
        RINOK(finalize());
    ...
    TEST_Z7_COM7F_IMF(SetOperationResult(Int32 operationResult)) {
        (void)operationResult;
        is_need_be_closed = true;
        return S_OK;
    }

    TEST_Z7_COM7F_IMF(GetVolumeSize(UInt32 index, UInt64 *size)) {
    ...
    TEST_Z7_COM7F_IMF(GetVolumeStream(UInt32 index, ISequentialOutStream **volumeStream)) {
        COM_TRY_BEGIN
        wchar_t temp[16]{};
    ...
    TEST_Z7_COM7F_IMF(CryptoGetTextPassword2(Int32 *passwordIsDefined, BSTR *password)) {
...
TEST_F(verify_7zip_command, a) {
    // Create a zip archive to /tmp/temp.zip from the current directory
...
class archive_open_callback Z7_final : public IArchiveOpenCallback,
    ...
    Z7_COM_ADDREF_RELEASE
public:
    TEST_Z7_COM7F_IMF(SetTotal(const UInt64 * /* files */, const UInt64 * /* bytes */)) { return S_OK; }

    TEST_Z7_COM7F_IMF(SetCompleted(const UInt64 * /* files */, const UInt64 * /* bytes */)) { return S_OK; }

    TEST_Z7_COM7F_IMF(CryptoGetTextPassword(BSTR *password)) {
...
TEST_F(verify_7zip_command, l) {
    // List archive items of a zip archive regular.zip
...
static HRESULT check_bool_archive_item_prop(IInArchive *archive, UInt32 index, PROPID prop_id, bool &result) {
    NWindows::NCOM::CPropVariant prop;
...
public:
    ...
    TEST_Z7_COM7F_IMF(SetTotal(UInt64 /* size */)) { return S_OK; }

    TEST_Z7_COM7F_IMF(SetCompleted(const UInt64 * /* completeValue */)) { return S_OK; }

    TEST_Z7_COM7F_IMF(GetStream(UInt32 index, ISequentialOutStream **outStream, Int32 askExtractMode)) {
        COM_TRY_BEGIN
        *outStream = nullptr;
    ...
    TEST_Z7_COM7F_IMF(PrepareOperation(Int32 askExtractMode)) {
        extract_mode = false;
    ...
    TEST_Z7_COM7F_IMF(SetOperationResult(Int32 opRes)) {
        switch (opRes) {
...
TEST_F(verify_7zip_command, x) {
    // Extract archive items of a zip archive regular.zip
